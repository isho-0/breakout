...
public:
    ...
    void SetFloat(
        const char *name, float value, bool useShader = false);

    void SetInteger(
        const char *name, int value, bool useShader = false);
    ...
    void SetVector4f(
        const char *name, float x, float y, float z, float w, bool useShader = false);
